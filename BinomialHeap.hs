...
{-
    *** TODO ***
    ...
    > findMin $ BinomialHeap.insert 3 'c' $ BinomialHeap.insert 2 'b' $ BinomialHeap.insert 1 'a' emptyHeap
    Just (1,'a')
-}
findMin :: Ord p => BinomialHeap p k -> Maybe (p, k)
findMin (BinomialHeap _ []) = Nothing
findMin heap = Just (minimumBy (compare `on` fst) [(prio, key) | Node prio key children <- trees heap])
...
{-
    ...
    * utilizați funcția zipExtend pt a facilita aplicarea unor funcționale.
    ...
    > test [Node 1 'a' []] [Node 2 'b' []]
    [ EmptyTree
    , Node {prio = 1, key = 'a', children = [Node {prio = 2, key = 'b', children = []}]}
    ]

    > test [Node 3 'c' []] $ test [Node 1 'a' []] [Node 2 'b' []]
    [ Node {prio = 3, key = 'c', children = []}
    , Node {prio = 1, key = 'a', children = [Node {prio = 2, key = 'b', children = []}]}
    ]
-}
...
mergeTrees trees1 trees2 = foldr add2List [] listOfPairs
    where
        listOfPairs = zipExtend EmptyTree EmptyTree trees1 trees2
        add2List (EmptyTree, EmptyTree) acc = EmptyTree : acc
        add2List (EmptyTree, tree) acc = tree : acc
        add2List (tree, EmptyTree) acc = tree : acc
        add2List (x, y) acc = EmptyTree : insertTree (attach x y) acc
...
-}
isolate :: a -> [a] -> [(a, [a])]
isolate placeHolder list = zip list modifiedList
  where
    modifiedList = map modify (zip (inits list) (drop 1 (tails list)))
    modify (x, []) = x
    modify (x, y) = x ++ [placeHolder] ++ y

{-
alt attempt, dar mai putine functionale + recursivitate

test placeHolder list = zip list modifiedList
    where
        replace left [[]] = [head left]
        replace left right = ((head left) ++ [placeHolder] ++ head(right)) : replace (tail left) (tail right)
        modifiedList = replace (inits list) (drop 1 (tails list))
-}
...
removeMin heap
  | size heap - 1 == 0 = emptyHeap
  | otherwise = BinomialHeap (size heap - 1) finalTree
  where
    isolated = isolate EmptyTree (trees heap)
    Just (x, y) = find (\(x, y) -> x == min) isolated
    min = (minimumBy (compare `on` prio) [nod | nod@(Node prio key children) <- trees heap])
    finalTree = mergeTrees (reverse $ children x) y
{-
    *** TODO ***
...
instance (Show p, Show k) => Show (BinomialTree p k) where
    ...
    show tree = intercalate "\n" $ showTree tree
      where
        showTree (Node prio key children) = (show prio ++ " (" ++ show key ++ ")") : map (replicate 2 ' ' ++) (concatMap showTree children)
...
instance Functor (BinomialTree p) where
    ...
    fmap f (Node prio key children) = Node prio (f key) (map (fmap f) children)
...
instance Foldable (BinomialTree p) where
    ...
    foldr f acc (Node prio key children) = f key (function acc)
      where function = foldr (.) id (map (flip $ foldr f) children)
  
-- aplic flip $ foldr f pe copii pentru a obtine o lista de functii de tipul b -> b (flip e pus ca sa pun copii pe primul parametru)
-- apoi aplic foldr (.) id pe lista de functii pentru a obtine o singura functie de tipul b -> b
-- in final, aplic functia rezultat pe acc